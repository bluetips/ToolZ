...
def crawl(keyword, startime, endtime, industry):
    ...
    pn = 1
    search_url = 'https://www.baidu.com/s?ie=utf-8&f=8&rsv_bp=1&tn=baidu&wd={}&si=(finance.qq.com)&ct=2097152&rqlang=cn&rsv_enter=1&gpc=stf={},{}|stftype=1&tfflag=1'.format(
        keyword, startime, endtime)
    while 1:
        refer = 'https://xueqiu.com/k?q={}'.format(keyword.encode('utf-8'))
        resp = getpage(search_url, headers, refer)
        ...
        for o in obj_list:
            ...
            try:
                mysql.insertOne(insert, (
                    keyword, industry, '第一财经', item['user'], datetime.datetime.now().date(), item['title'],
                    item['content'],
