...
def crawl(keyword, startime, endtime, industry):
    ...
    headers = {
        ...
        'Accept': '*/*',
        'Referer': 'http://so.eastmoney.com/news/s?keyword=%E5%8D%97%E4%BA%AC'
    }
    pn = 1
    pre_resp = None

    while 1:
        search_url = 'http://api.so.eastmoney.com/bussiness/Web/GetSearchList?type=20&pageindex={}&pagesize=10&keyword={}&name=normal'.format(
            pn, keyword)
        refere = 'http://so.eastmoney.com/news/s?keyword={}'.format(keyword.encode('utf-8'))
        resp = getpage(search_url, headers, refere)
        if pre_resp == resp:
            break
        pre_resp = resp
        try:
            if json.loads(resp.content.decode())['Code'] == 1:
                break
        except Exception:
            pass

        obj_list = json.loads(resp.content.decode())['Data']
        print(len(obj_list))
        for o in obj_list:
            ctime = o['Art_CreateTime']
            ...
            if 'life.' in url:
                # 时尚频道
                content = ''.join(ele.xpath('//div[@id="article-body"]//text()'))
                item['content'] = emoji.demojize(content).strip()
                item['url'] = url
                item['post_date'] = re.findall(r'com/a/(\d{8}).html', url)
                try:
                    item['user'] = ele.xpath('//span[@class="article-meta-cate"]//text()')[0]
                    item['user'] = re.sub(r'[\s\r\n]', '', item['user'])
                except Exception as e:
                    item['user'] = ''
                item['title'] = ele.xpath('//h1[@class="article-title"]//text()')[0]
            else:

                content = ''.join(ele.xpath('//div[@id="ContentBody"]//text()'))
                item['content'] = emoji.demojize(content).strip()
                item['url'] = url
                item['post_date'] = ele.xpath('//div[@class="time"]/text()')[0].replace('年', '-').replace('月',
                                                                                                          '-').replace(
                    '日', '')
                try:
                    item['user'] = ''.join(ele.xpath('//div[@class="source data-source"]//text()')).replace('来源',
                                                                                                            '').strip()
                    item['user'] = re.sub(r'[\s\r\n]', '', item['user'])
                except Exception as e:
                    item['user'] = ''
                item['title'] = ele.xpath('//div[@class="newsContent"]/h1//text()')[0]
            ...
            try:
                mysql.insertOne(insert, (
                    keyword, industry, '东方财富网', item['user'], datetime.datetime.now().date(), item['title'],
                    item['content'],
                    item['post_date'], sentiment, score, url, sensitivity, check))
                mysql.end()
                print(item['title'], item['user'])
            except Exception as e:
                print(e)
                pass
        pn += 1
