...
def pie():
    ...
    data = jsonify({'data {}'.format(name): rr(1, 100) for name in letters})
    return data
...
@cross_origin()
@app.route('/price_bar', methods=['GET'])
def price_bar():
    c1 = client['jd']['jd_goods']
    c2 = client['jd']['jd_comments']
    data_list = []
    for i in range(20):
        data = c1.count_documents({'$and': [{'price': {'$gt': i * 50}}, {'price': {'$lte': i * 50 + 50}}]})
        item = {
            '价格': '{}-{}'.format(str(i * 50), str(i * 50 + 50)),
            '宝贝数': data
        }
    ...
    json_data['data']['values'] = data_list
    return jsonify(json_data)


"""
饼状图返回 
'{"data a": 7, "data b": 60, "data c": 77, "data d": 11, "data e": 44}'
jsonify(dict)
"""


@cross_origin()
@app.route('/brand_pie', methods=['GET'])
# mongo聚合查询 pipe = [{'$group' : {'_id' : "$title", 'num_tutorial' : {'$sum' : 1}}}]
def brand_pie():
    c1 = client['jd']['jd_goods']
    c2 = client['jd']['jd_comments']
    pipe = [{'$group': {'_id': "$brand", 'num_tutorial': {'$sum': 1}}}, {'$sort': {'num_tutorial': -1}}, {'$limit': 11}]
    ret = c1.aggregate(pipe)
    # ret = c1.find()
    item = {}
    for i in ret:
        if i['_id'] == None or i['_id'] == '无品牌':
            continue
        item[i['_id']] = i['num_tutorial']

    return jsonify(item)


@cross_origin()
@app.route('/top50_goods_bar', methods=['GET'])
# mongo聚合查询 pipe = [{'$group' : {'_id' : "$title", 'num_tutorial' : {'$sum' : 1}}}]
def top50_goods_bar():
    c1 = client['jd']['jd_goods']
    ret = c1.find().sort([('comment_data.CommentCount', pymongo.DESCENDING)]).limit(10)
    num = 0
    data_list = []
    for i in ret:
        num += 1
        item = {
            '宝贝': i['title'],
            '销量': i['comment_data']['CommentCount']
        }
        # todo 继续做
        data_list.append(item)
        if num == 50:
...
def get_cut_ret():
    c1 = client['jd']['jd_goods']
    c2 = client['jd']['jd_goods_cut']
    ret = c1.find().sort([('num_tutorial', -1), ]).limit(100)
    title = ''
    for i in ret:
        title = title + i['title']
    cut_ret = jieba.analyse.extract_tags(title, topK=100, withWeight=True)
    item_list = []
    for i in cut_ret:
        item = {}
        item['text'] = i[0]
...
@cross_origin()
@app.route('/title_word', methods=['GET'])
def title_word():
    c2 = client['jd']['jd_goods_cut']
    ret = c2.find()
    item_list = []
    for i in ret:
        item = {}
        if i['text'] == '精华液' or i['text'] == '精华':
            continue
        item['text'] = i['text']
        item['size'] = int(i['size'] * 500)
        item_list.append(item)
